...
def populate_surah_data():
    ...
    if r:
        ...
        filtered_surah_data = [{k:(v if isinstance(v, int) else v.replace("Al", "", 1).replace("-","")) for k,v in surah.items() if k == 'englishName' or k == 'numberOfAyahs'} for surah in surah_data]
        global  surah_list
